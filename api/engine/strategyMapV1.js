...
function computeModeScores(signals) {
  ...
  if (signals.emotional_volatility === "high") {
    escalation += 25;
    addDriver(drivers, {
      source_signal: "emotional_volatility=high",
      affected_mode: "escalation_risk",
      delta: 25,
      reason: "High volatility increases escalation probability."
    });
  }
  if (signals.continuity === "high") {
    escalation += 20;
    addDriver(drivers, {
      source_signal: "continuity=high",
      affected_mode: "escalation_risk",
      delta: 20,
      reason: "Ongoing interaction increases escalation probability."
    });
  }
  if (signals.prior_conflict === true) {
    escalation += 15;
    addDriver(drivers, {
      source_signal: "prior_conflict=true",
      affected_mode: "escalation_risk",
      delta: 15,
      reason: "Prior conflict raises escalation likelihood."
    });
  }
  if (signals.urgency_level === "high") {
    escalation += 15;
    addDriver(drivers, {
      source_signal: "urgency_level=high",
      affected_mode: "escalation_risk",
      delta: 15,
      reason: "High urgency tends to compress tone control."
    });
  }
  if (signals.power_asymmetry === true) {
    escalation += 10;
    addDriver(drivers, {
      source_signal: "power_asymmetry=true",
      affected_mode: "escalation_risk",
      delta: 10,
      reason: "Power imbalance increases escalation sensitivity."
    });
  }
  ...
  if (signals.audience_multiparty === true) {
    ...
    addDriver(drivers, {
      source_signal: "audience_multiparty=true",
      affected_mode: "misinterpretation_risk",
      delta: 20,
      reason: "More observers increase misreading risk."
    });
  }
  if (signals.emotional_volatility === "high") {
    ...
    addDriver(drivers, {
      ...
      affected_mode: "misinterpretation_risk",
      delta: 15,
      reason: "High volatility increases ambiguity and misreads."
    });
  }
  if (signals.relationship_axis === "peripheral") {
    misinterpretation += 15;
    addDriver(drivers, {
      source_signal: "relationship_axis=peripheral",
      affected_mode: "misinterpretation_risk",
      delta: 15,
      reason: "Lower shared context increases misinterpretation risk."
    });
  }
  if (signals.written_record_expected === true) {
    misinterpretation += 10;
    addDriver(drivers, {
      source_signal: "written_record_expected=true",
      affected_mode: "misinterpretation_risk",
      delta: 10,
      reason: "Written records amplify interpretation and replay."
    });
  }
  if (signals.urgency_level === "high") {
    misinterpretation += 10;
    addDriver(drivers, {
      ...
      affected_mode: "misinterpretation_risk",
      delta: 10,
      reason: "Urgency reduces clarity and increases misreads."
    });
  }
  ...
  if (signals.written_record_expected === true) {
    ...
    addDriver(drivers, {
      source_signal: "written_record_expected=true",
      affected_mode: "documentation_backfire_risk",
      delta: 30,
      reason: "Documentation increases record-based blowback risk."
    });
  }
  if (signals.legal_or_liability_context === true) {
    documentation += 25;
    addDriver(drivers, {
      source_signal: "legal_or_liability_context=true",
      affected_mode: "documentation_backfire_risk",
      delta: 25,
      reason: "Liability context increases documentation backfire risk."
    });
  }
  if (signals.power_asymmetry === true) {
    documentation += 10;
    addDriver(drivers, {
      ...
      affected_mode: "documentation_backfire_risk",
      delta: 10,
      reason: "Power imbalance increases documentation sensitivity."
    });
  }
  ...
  if (signals.relationship_axis === "intimate") {
    relationship += 25;
    addDriver(drivers, {
      source_signal: "relationship_axis=intimate",
      affected_mode: "relationship_break_risk",
      delta: 25,
      reason: "Intimate relationships have higher rupture stakes."
    });
  } else if (signals.relationship_axis === "personal") {
    relationship += 20;
    addDriver(drivers, {
      source_signal: "relationship_axis=personal",
      affected_mode: "relationship_break_risk",
      delta: 20,
      reason: "Personal relationships increase rupture sensitivity."
    });
  }
  if (signals.prior_conflict === true) {
    relationship += 15;
    addDriver(drivers, {
      ...
      affected_mode: "relationship_break_risk",
      delta: 15,
      reason: "Prior conflict increases break risk."
    });
  }
  if (signals.continuity === "high") {
    relationship += 10;
    addDriver(drivers, {
      ...
      affected_mode: "relationship_break_risk",
      delta: 10,
      reason: "Ongoing continuity raises relationship stakes."
    });
  }
  ...
  if (signals.legal_or_liability_context === true) {
    ...
    addDriver(drivers, {
      source_signal: "legal_or_liability_context=true",
      affected_mode: "admission_risk",
      delta: 35,
      reason: "Liability context increases admission risk."
    });
  }
  if (signals.written_record_expected === true) {
    admission += 20;
    addDriver(drivers, {
      ...
      affected_mode: "admission_risk",
      delta: 20,
      reason: "Written records increase admission exposure."
    });
  }
  if (signals.power_asymmetry === true) {
    admission += 15;
    addDriver(drivers, {
      ...
      affected_mode: "admission_risk",
      delta: 15,
      reason: "Power imbalance increases admission consequences."
    });
  }
  if (signals.relationship_axis === "peripheral") {
    admission += 10;
    addDriver(drivers, {
      ...
      affected_mode: "admission_risk",
      delta: 10,
      reason: "Peripheral relationships increase formality and admission exposure."
    });
  }
  ...
  if (escalation >= 60) {
    relationship += 15;
    addDriver(drivers, {
      source_signal: "derived:escalation>=60",
      affected_mode: "relationship_break_risk",
      delta: 15,
      reason: "High escalation elevates relationship break risk."
    });
  }
...
function computeOverallTier(scores) {
  ...
  // Override rules (v1.1 tuning)
  if (scores.admission_risk >= 70) overall = "extreme";
  if (scores.documentation_backfire_risk >= 70) overall = "extreme";
  if (scores.escalation_risk >= 70 && scores.relationship_break_risk >= 50) overall = "extreme";
